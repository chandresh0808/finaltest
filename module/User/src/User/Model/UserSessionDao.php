...
class UserSessionDao extends \Application\Model\AbstractDao
{
    ...
    {
        //UPDATE user_session SET delete_flag=1 WHERE delete_flag='0' AND FROM_UNIXTIME(last_request_dt_tm) <= DATE_SUB(NOW(), INTERVAL 10 MINUTE);
        $query = "UPDATE user_session SET delete_flag=1 WHERE 
            delete_flag='0' AND updated_dt_tm <= DATE_SUB(NOW(), INTERVAL 10 MINUTE)";
