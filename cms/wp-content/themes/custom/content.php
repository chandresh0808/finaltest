...
?>
	...
	<article id="post-<?php the_ID(); ?>" <?php post_class( $featured_image ); ?>>
		<div class="row">
			<?php
			$align = get_post_meta( get_the_ID(), 'custom_home_page_alignment', true );
			$col = 'col-md-12';
			if( has_post_thumbnail() && ! is_single() && 'excerpt' == $custom_theme_options['excerpt_content'] ) {
			...
			<div class="<?php echo $col; ?>">
			    ...
			    <div class="entry-content">
				    <?php
					if ( 'excerpt' == $custom_theme_options['excerpt_content'] && empty( $format ) && ( ! is_single() || is_search() || is_archive() ) ) {
						the_excerpt();
					} else {
						the_content( __( 'Read more &#133;', 'custom' ) );
					}
					?>
			    </div><!-- .entry-content -->
